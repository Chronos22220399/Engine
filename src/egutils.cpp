...
Egutils::TextProcessor::TextProcessor()
...
std::set<std::string>
Egutils::TextProcessor::getIntersection(std::string const &left,
...
std::string Egutils::TextProcessor::strip(std::string const &str) const {
...
std::string Egutils::TextProcessor::process(std::string const &text) const {
	return strip(text);
}

std::string
Egutils::TextProcessor::setToString(std::set<std::string> const &s) const {
	std::string ret{};
...
std::string
Egutils::TextProcessor::advancedProcess(std::string const &text) const {
	using namespace std;
	...
	while (end != string::npos) {
		...
		// ""，这满足计算公式，因此无需额外考虑
		auto right{
			strip(stripedText.substr(next_begin, next_end - next_begin))};
		if (op == "|") {
			stk.push(right);
		} else if (op == "&") {
			auto left = stk.top();
...
void Egutils::CharUtils::extractChar(
